...
class GoreCalculator {
    ...
    evaluarExpresion(expresion) {
        // Reemplazar π por su valor
        expresion = expresion.replace(/π/g, Math.PI);
        
        // Evaluar funciones científicas
        expresion = expresion.replace(/sin\(([^)]+)\)/g, (match, angulo) => {
            const valor = eval(angulo);
            return Math.sin(this.gradosARadianes(valor));
        });
        
        expresion = expresion.replace(/cos\(([^)]+)\)/g, (match, angulo) => {
            const valor = eval(angulo);
            return Math.cos(this.gradosARadianes(valor));
        });
        
        expresion = expresion.replace(/√\(([^)]+)\)/g, (match, numero) => {
            const valor = eval(numero);
            if (valor < 0) throw new Error("Raíz de número negativo");
    ...
    handleButtonClick(boton) {
        ...
        if(boton.id === "c") {
            ...
            this.addGoreEffect("RESET");
            return;
        }

        // Botón Borrar
        if (boton.id === "borrar") {
            if (this.pantalla.textContent.length === 1 || this.pantalla.textContent === "ERROR 666") {
                this.pantalla.textContent = "0";
            } else {
                this.pantalla.textContent = this.pantalla.textContent.slice(0, -1);
            }
            this.addGoreEffect("DELETE");
            return;
        }

        // Funciones científicas
        if (boton.classList.contains('function')) {
            const funcion = boton.getAttribute('data-function');
            
            if (funcion === 'pi') {
                if (this.pantalla.textContent === "0" || this.pantalla.textContent === "ERROR 666") {
                    this.pantalla.textContent = "π";
                } else {
                    this.pantalla.textContent += "π";
                }
                this.addGoreEffect("PI");
                return;
            }
            ...
            if (this.pantalla.textContent === "0" || this.pantalla.textContent === "ERROR 666") {
                this.pantalla.textContent = funcionTexto;
            } else {
                this.pantalla.textContent += funcionTexto;
            }
            this.addGoreEffect("FUNCTION");
            return;
        }
        ...
        if (boton.id === "igual") {
            try {
                const resultado = this.evaluarExpresion(this.pantalla.textContent);
                
                if (isNaN(resultado) || !isFinite(resultado)) {
                    this.pantalla.textContent = "ERROR 666";
                    this.addGoreEffect("ERROR");
                } else {
                    this.pantalla.textContent = this.formatearResultado(resultado);
                    this.addGoreEffect("CALCULATE");
                }
            } catch {
                this.pantalla.textContent = "ERROR 666";
                this.addGoreEffect("ERROR");
        ...
        if(this.pantalla.textContent === "0" || this.pantalla.textContent === "ERROR 666") {
            if (['+', '-', '×', '÷', ')'].includes(botonApretado)) {
                this.pantalla.textContent = "0" + botonApretado;
            } else {
        ...
        this.addGoreEffect("INPUT");
    }
    
    addGoreEffect(type) {
        switch(type) {
            case "ERROR":
                this.pantalla.style.animation = "errorFlash 0.5s ease";
                break;
            case "CALCULATE":
                this.pantalla.style.animation = "calculateGlow 0.8s ease";
                break;
            case "RESET":
                this.pantalla.style.animation = "resetPulse 0.6s ease";
                break;
            case "DELETE":
                this.pantalla.style.animation = "deleteFade 0.3s ease";
                break;
            default:
                this.pantalla.style.animation = "inputFlicker 0.2s ease";
                break;
        }
        
        // Limpiar animación después de que termine
        setTimeout(() => {
            this.pantalla.style.animation = '';
        }, 1000);
    }
    ...
    setupKeyboardSupport() {
        document.addEventListener('keydown', (event) => {
            ...
            if (key >= '0' && key <= '9') {
                const boton = Array.from(this.botones).find(b => b.textContent === key);
                if (boton) boton.click();
            } else if (key === '+' || key === '-') {
                const boton = Array.from(this.botones).find(b => b.textContent === key);
                if (boton) boton.click();
            } else if (key === '*') {
                const boton = Array.from(this.botones).find(b => b.textContent === '×');
                if (boton) boton.click();
            } else if (key === '/') {
                ...
                const boton = document.getElementById('igual');
                if (boton) boton.click();
            } else if (key === 'Escape' || key.toLowerCase() === 'c') {
                const boton = document.getElementById('c');
                if (boton) boton.click();
            ...
            // Atajos de teclado para funciones científicas
            else if (key.toLowerCase() === 's') {
                const boton = Array.from(this.botones).find(b => b.getAttribute('data-function') === 'sin');
                if (boton) boton.click();
            } else if (key.toLowerCase() === 'o') {
                const boton = Array.from(this.botones).find(b => b.getAttribute('data-function') === 'cos');
                if (boton) boton.click();
            } else if (key.toLowerCase() === 'r') {
                const boton = Array.from(this.botones).find(b => b.getAttribute('data-function') === 'sqrt');
                if (boton) boton.click();
            } else if (key.toLowerCase() === 'p') {
                const boton = Array.from(this.botones).find(b => b.getAttribute('data-function') === 'pi');
                if (boton) boton.click();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const calculator = new GoreCalculator();
...
// Animaciones CSS dinámicas para la calculadora gore
const style = document.createElement('style');
style.textContent = `
    @keyframes errorFlash {
        0%, 100% { background: linear-gradient(145deg, #000, #1a0000); }
        50% { background: linear-gradient(145deg, #660000, #990000); }
    }
    
    @keyframes calculateGlow {
        0% { 
            box-shadow: inset 0 5px 15px rgba(0, 0, 0, 0.5), 0 0 20px rgba(255, 0, 0, 0.2);
        }
        50% { 
            box-shadow: inset 0 5px 15px rgba(0, 0, 0, 0.5), 0 0 40px rgba(0, 255, 0, 0.6);
        }
        100% { 
            box-shadow: inset 0 5px 15px rgba(0, 0, 0, 0.5), 0 0 20px rgba(255, 0, 0, 0.2);
        }
